continent_dict = {
    # Africa
    'Angola': 'Africa', 'Algeria': 'Africa', 'Benin': 'Africa', 'Botswana': 'Africa',
    'Burkina Faso': 'Africa', 'Burundi': 'Africa', 'Cape Verde': 'Africa', 'Cameroon': 'Africa',
    'Chad': 'Africa', 'Comoros': 'Africa', 'Republic of the Congo': 'Africa',
    'Democratic Republic of the Congo': 'Africa', 'Ivory Coast': 'Africa', 'Egypt': 'Africa',
    'Eritrea': 'Africa', 'Ethiopia': 'Africa', 'Gabon': 'Africa', 'Gambia': 'Africa', 'Ghana': 'Africa',
    'Guinea': 'Africa', 'Equatorial Guinea': 'Africa', 'Guinea-Bissau': 'Africa',
    'Kenya': 'Africa', 'Lesotho': 'Africa', 'Liberia': 'Africa', 'Libya': 'Africa', 'Madagascar': 'Africa',
    'Malawi': 'Africa', 'Mali': 'Africa', 'Morocco': 'Africa', 'Mauritius': 'Africa', 'Mauritania': 'Africa',
    'Mozambique': 'Africa', 'Namibia': 'Africa', 'Niger': 'Africa', 'Nigeria': 'Africa',
    'Central African Republic': 'Africa', 'Rwanda': 'Africa', 'Senegal': 'Africa',
    'Seychelles': 'Africa', 'Sierra Leone': 'Africa', 'Somalia': 'Africa', 'South Africa': 'Africa',
    'Sudan': 'Africa', 'South Sudan': 'Africa', 'Eswatini': 'Africa', 'Tanzania': 'Africa',
    'Togo': 'Africa', 'Tunisia': 'Africa', 'Uganda': 'Africa', 'Djibouti': 'Africa', 'Zambia': 'Africa',
    'Zimbabwe': 'Africa',

    # America
    'Bermuda': 'America', 'Canada': 'America', 'United States': 'America',
    'Greenland': 'America', 'Mexico': 'America', 'Saint Pierre and Miquelon': 'America',
    'Argentina': 'America', 'Bolivia': 'America', 'Brazil': 'America', 'Chile': 'America',
    'Colombia': 'America', 'Ecuador': 'America', 'French Guiana': 'America', 'Guyana': 'America',
    'Paraguay': 'America', 'Peru': 'America', 'Suriname': 'America', 'Uruguay': 'America',
    'Venezuela': 'America', 'Belize': 'America', 'Costa Rica': 'America', 'El Salvador': 'America',
    'Guatemala': 'America', 'Honduras': 'America', 'Nicaragua': 'America', 'Panama': 'America',
    'Cuba': 'America', 'Dominican Republic': 'America', 'Puerto Rico': 'America', 'Jamaica': 'America',
    'Trinidad and Tobago': 'America', 'Barbados': 'America', 'Bahamas': 'America', 'Haiti': 'America',
    'Grenada': 'America', 'Saint Vincent and the Grenadines': 'America', 'Saint Lucia': 'America',
    'Saint Kitts and Nevis': 'America',

    # Asia
    'Afghanistan': 'Asia', 'Saudi Arabia': 'Asia', 'Armenia': 'Asia', 'Azerbaijan': 'Asia',
    'Bangladesh': 'Asia', 'Bahrain': 'Asia', 'Myanmar': 'Asia', 'Bhutan': 'Asia', 'Brunei': 'Asia',
    'Cambodia': 'Asia', 'Qatar': 'Asia', 'China': 'Asia', 'North Korea': 'Asia',
    'South Korea': 'Asia', 'United Arab Emirates': 'Asia', 'Philippines': 'Asia', 'Georgia': 'Asia',
    'India': 'Asia', 'Indonesia': 'Asia', 'Iraq': 'Asia', 'Iran': 'Asia', 'Israel': 'Asia',
    'Japan': 'Asia', 'Jordan': 'Asia', 'Kazakhstan': 'Asia', 'Kyrgyzstan': 'Asia', 'Kuwait': 'Asia',
    'Laos': 'Asia', 'Lebanon': 'Asia', 'Malaysia': 'Asia', 'Maldives': 'Asia', 'Mongolia': 'Asia',
    'Nepal': 'Asia', 'Oman': 'Asia', 'Pakistan': 'Asia', 'Palestine': 'Asia', 'Russia': 'Asia',
    'Singapore': 'Asia', 'Syria': 'Asia', 'Sri Lanka': 'Asia', 'Thailand': 'Asia', 'Tajikistan': 'Asia',
    'Turkmenistan': 'Asia', 'Turkey': 'Asia', 'Uzbekistan': 'Asia', 'Vietnam': 'Asia', 'Yemen': 'Asia',

    # Europe
    'Albania': 'Europe', 'Germany': 'Europe', 'Andorra': 'Europe', 'Austria': 'Europe',
    'Belgium': 'Europe', 'Belarus': 'Europe', 'Bosnia and Herzegovina': 'Europe', 'Bulgaria': 'Europe',
    'Cyprus': 'Europe', 'Croatia': 'Europe', 'Denmark': 'Europe', 'Slovakia': 'Europe',
    'Slovenia': 'Europe', 'Spain': 'Europe', 'Estonia': 'Europe', 'Finland': 'Europe',
    'France': 'Europe', 'Greece': 'Europe', 'Hungary': 'Europe', 'Ireland': 'Europe', 'Iceland': 'Europe',
    'Italy': 'Europe', 'Kosovo': 'Europe', 'Latvia': 'Europe', 'Liechtenstein': 'Europe',
    'Lithuania': 'Europe', 'Luxembourg': 'Europe', 'North Macedonia': 'Europe', 'Malta': 'Europe',
    'Moldova': 'Europe', 'Norway': 'Europe', 'Netherlands': 'Europe', 'Poland': 'Europe',
    'Portugal': 'Europe', 'United Kingdom': 'Europe', 'Czech Republic': 'Europe', 'Romania': 'Europe',
    'San Marino': 'Europe', 'Serbia': 'Europe', 'Sweden': 'Europe', 'Switzerland': 'Europe', 'Ukraine': 'Europe',

    # Oceania
    'Australia': 'Oceania', 'Fiji': 'Oceania', 'Cook Islands': 'Oceania', 'Marshall Islands': 'Oceania',
    'Solomon Islands': 'Oceania', 'Micronesia': 'Oceania', 'Nauru': 'Oceania', 'New Zealand': 'Oceania',
    'Palau': 'Oceania', 'Papua New Guinea': 'Oceania', 'Samoa': 'Oceania', 'Tonga': 'Oceania',
    'Tuvalu': 'Oceania', 'Vanuatu': 'Oceania'
}

...
class handleData(BaseEstimator, TransformerMixin):
    ...
    # Function to replace outliers with NaN based on a confidence interval (IC)
    def replace_outliers_with_nan(df, confidence_interval=0.99):
        """
        ...
        if not (0 < confidence_interval < 1):
            raise ValueError("Confidence interval must be a float between 0 and 1.")
        ...
        for column in transformed_df.select_dtypes(include=[np.number]).columns:
            lower_limit = transformed_df[column].quantile(lower_percentile)
            upper_limit = transformed_df[column].quantile(upper_percentile)
        ...
        # Calculate the total number of cells and the percentage replaced
        total_cells = transformed_df.size
        percentage_replaced = (cells_replaced / total_cells) * 100
        
        return transformed_df, cells_replaced, percentage_replaced